...
**Response:** We agree. All percentage-based F-statistic comparisons have been removed. The Atlas1006 section now simply reports F-statistics and p-values for each method without computing percentage differences: "MeLSI (F = 4.841, p = 0.005), Euclidean (F = 4.711, p = 0.001), Bray-Curtis (F = 4.442, p = 0.001), Jaccard (F = 1.791, p = 0.144)." We note that the MeLSI F-statistic differs slightly from the reviewer's cited value of 5.141: Figures 1-3 were regenerated with a fixed random seed (set.seed(42)) for full reproducibility. Because MeLSI uses an ensemble of random subspace projections, results vary slightly across independent runs; both values yield the same significance level (p = 0.005) and the same biological conclusion. We have also added the caveat that "F-statistics from different distance metrics are not directly comparable because each defines a distinct geometric space." The Conclusions section has been revised to remove all percentage improvement claims, focusing instead on empirical power from Table 2 and the interpretability advantage.
...
For SKIOME, Bray-Curtis yields a substantially larger pseudo-F statistic (F=16.275 vs MeLSI F=4.972), but as Reviewer 1 correctly noted, F-statistics from different distance metrics are not directly comparable because each defines a distinct geometric space. The most informative comparison is MeLSI (F=4.972) versus Euclidean (F=4.897), since both operate in CLR-transformed space. That MeLSI and Euclidean perform essentially identically is itself a meaningful biological finding: it indicates the signal in the SKIOME dataset is broadly distributed across many taxa, rather than concentrated in a small subset. In such cases, a weighted metric offers no detection advantage — and MeLSI correctly learns this, assigning relatively uniform weights across taxa rather than artificially inflating a few. This behavior reflects statistical integrity, not a limitation. Crucially, while Bray-Curtis provides only an omnibus p-value, MeLSI additionally provides learned feature weights identifying which taxa drive group separation (e.g., Staphylococcus as the top-weighted taxon, consistent with its known role in atopic dermatitis), enabling biological interpretation that no fixed-distance method can provide within a single unified framework.
...
**Response:** We have added a new paragraph in the Conclusions explaining the mechanism in depth. The key insight is that standard PERMANOVA with fixed distance metrics treats all taxa as equally informative. In high-dimensional microbiome data, where the number of taxa often exceeds the sample size, this is problematic: uninformative taxa contribute noise to the distance calculation, diluting the signal from the small number of taxa that actually differ between groups. MeLSI addresses this by learning diagonal metric weights $M_{jj}$ that scale each taxon's contribution to the pairwise distance. Taxa with strong between-group differences receive higher weights; noise taxa receive lower weights. The resulting weighted Mahalanobis distance concentrates statistical power on the most informative features — analogous to feature selection, but embedded directly in the distance metric and optimized jointly with the test statistic via ensemble gradient descent. This mechanism is why MeLSI can detect signals that fixed metrics miss: the DietSwap dataset illustrates this concretely, where MeLSI achieved significance (p=0.015) while Euclidean, Bray-Curtis, and Jaccard remained marginal (p≥0.066). The learned weights also provide a direct biological output — a ranked list of taxa driving group separation — whereas fixed distance methods provide only an omnibus p-value, requiring separate downstream analyses (e.g., differential abundance testing) to identify contributing taxa. This unification of hypothesis testing and feature attribution within a single permutation framework is MeLSI's core methodological contribution.
